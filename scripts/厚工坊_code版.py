...
功能：
  1. 四端口本地服务获取微信 code
  2. /YUN/api/onLogin.aspx 使用 code 换 SessionID（Cookie 会话）
  3. 每日签到
  4. PushPlus 推送
...
def common_headers(session_id: str | None = None) -> Dict[str, str]:
    ...
    if session_id:
        ...
        headers["Cookie"] = f"user=SessionID={session_id}"
    return headers


def api_request(server: str, url: str, session_id: str | None, method: str = "POST",
                query: Dict[str, Any] | None = None, proxies: Dict[str, str] | None = None) -> Dict[str, Any]:
    qs = f"?{form_encode(query)}" if query else ""
    response = request_with_proxy(
        method, f"{url}{qs}", headers=common_headers(session_id), json={} if method == "POST" else None,
        proxies=proxies, server=server,
    )
    try:
        return response.json()
    except Exception:
        return {"status": -1, "msg": f"JSON解析失败/HTTP {response.status_code}: {response.text[:300]}"}
...
class AccountSession:
    """单个 code 服务地址的会话: SessionID（sessionid 头 + user Cookie）"""

    def __init__(self, server: str):
        self.server = server
        self.token = ""
        # 设备号按 code 服务地址稳定派生：同一账号每次跑都一样，避免被当成新设备
        self.device_id = "d_" + __import__("hashlib").md5(server.encode("utf-8")).hexdigest()[:16]
    ...
    def login(self) -> None:
        ...
        res = api_request(self.server, LOGIN_URL, None, method="GET", query={"opt": "onLogin", "code": code})
        if not is_ok(res):
            raise RuntimeError(f"登录失败: {msg_of(res)}")
        self.token = str(res.get("SessionID") or res.get("sessionID") or safe_data(res).get("SessionID") or "")
        if not self.token:
    ...
    def sign(self, retry: bool = True) -> str:
        vers = datetime.utcnow().strftime("%Y%m%d")
        res = api_request(self.server, SIGN_URL, self.token, method="POST",
                          query={"op": "now", "vers": vers})
        if is_ok(res):
            print("✅ [签到] 签到成功")
            return "签到成功"
