...
⚠️ 源脚本为抓包 Authorization（Bearer）型，登录接口为推断，
...
BASE_URL = "https://api.hzyxhfp.com"
LOGIN_URL = f"{BASE_URL}/api/users/wxLogin"
USER_DETAIL_URL = f"{BASE_URL}/api/users/getUserDetailInfo"
SIGN_IN_URL = f"{BASE_URL}/api/signInLog/addSignIn"
POINTS_URL = f"{BASE_URL}/api/userRight/getUserRightDetail"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hzhfpcookie.json")
...
def common_headers(token: str | None = None) -> Dict[str, str]:
    headers = {
        ...
        "Accept": "*/*",
        "app": APPID,
        "Referer": f"https://servicewechat.com/{APPID}/377/page-frame.html",
    }
    if token:
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（用户详情接口验证），失效自动 code 刷新"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            detail_resp = api_post(server, USER_DETAIL_URL, cache_token, proxies, {})
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if info_resp.get("code") != 0:
            ...
            phone = str(info_data.get("phone") or "")
            nickname = info_data.get("wx_nickname") or "-"
            result["user"] = f"{nickname}({mask_phone(phone)})"
            print(f"👤 [用户] {nickname}({mask_phone(phone)})")
        ...
        if sign_resp.get("code") != 0:
            ...
            cont_days = safe_data(sign_resp).get("cont_days")
            result["signMsg"] = f"签到成功！已连续签到{cont_days}天"
            print(f"✅ [签到] {result['signMsg']}")

        sleep(random.uniform(1.0, 1.5))

        points_resp = api_post(server, POINTS_URL, token, proxies, {})
        if points_resp.get("code") != 0:
            print(f"⚠️ [积分] 获取积分失败: {points_resp.get('msg') or json_preview(points_resp, 300)}")
        else:
            points_value = safe_data(points_resp).get("integral")
...
def main() -> None:
    ...
    send_pushplus("💎 媓钻护肤品小程序任务完成", build_notify(results))
