...
"""
沪上阿姨小程序签到动态 code 版（企迈 qmai 白标平台）

功能：
  1. 本地 code 服务获取微信 code（可选多端口多账号）
  2. /account-center/oauth/mini-app-login 使用 code 换 token（Qm-User-Token）
     —— 企迈 AES-GCM 加密登录（实证，同平台 qmai 脚本验证通过）；
     企迈全站接口（含签到/积分等业务接口）统一走 AES-GCM 加密请求
  3. 每日签到（/web/cmk-center/sign/takePartInSign，活动 id 每月 1 号可能变化，可用环境变量覆盖）
  4. 查询积分（/web/catering2-apiserver/crm/points-info）
  5. PushPlus 推送
...
环境变量：
  ...
  HSAY_BRAND_ID     企迈 brandId，登录报「品牌Id为空」时必填（抓包小程序登录请求体获取）
  CODE_SERVER       本地 code 服务地址，默认 127.0.0.1:8088
  HSAY_ACTIVITY_ID  签到活动 id（积分快失效的下月 1 号会变），默认 702822503017398273
  HSAY_SIGN_URL     签到接口地址，默认 https://webapi.qmai.cn/web/cmk-center/sign/takePartInSign
...
BRAND_ID = os.getenv("HSAY_BRAND_ID", "")  # qmai 平台 brandId（登录返回「品牌Id为空」时必填，抓包小程序请求体获取）
...
SIGN_URL = os.getenv("HSAY_SIGN_URL", f"{BASE_URL}/web/cmk-center/sign/takePartInSign")
ACTIVITY_ID = os.getenv("HSAY_ACTIVITY_ID", "702822503017398273")
...
STORE_ID = ""  # 沪上阿姨无固定门店 id，store-id 头留空

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "hsaycookie.json")
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        ...
        data = qmai_request(
            "POST",
            LOGIN_URL,
            {"code": code, "eVersion": "1.0", "brandId": BRAND_ID},
            proxies=proxies,
            server=server,
        )
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    ...
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            points_resp = api_post(server, POINTS_URL, cache_token, proxies, {"appid": APPID})
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sign_code = sign_resp.get("code")
        sign_msg = sign_resp.get("message") or sign_resp.get("msg") or ""
        if sign_code == 0 or sign_code == 400041:
            if sign_resp.get("status"):
                result["signMsg"] = "签到成功"
                print(f"✅ [签到] {result['signMsg']}")
            else:
                result["signMsg"] = sign_msg or "今日已签到"
                print(f"⚠️ [签到] {result['signMsg']}")
        else:
            result["signMsg"] = sign_msg or "签到失败"
            print(f"❌ [签到] {result['signMsg']}")

        points_resp = api_post(server, POINTS_URL, token, proxies, {"appid": APPID})
        if points_resp.get("code") != 0:
            print(f"⚠️ [积分] 获取积分失败: {points_resp.get('message') or points_resp.get('msg') or json_preview(points_resp, 300)}")
        total_points = safe_data(points_resp).get("totalPoints", 0)

        result["points"] = str(total_points)
        print(f"💰 [积分] 当前共 {total_points} 积分")
...
def main() -> None:
    ...
    send_pushplus("🥤 沪上阿姨签到任务完成", build_notify(results))
