...
⚠️ 登录接口为推断，未经真机验证，失败请抓包核对（源脚本为抓包 token 型，无登录调用；
   此版按鸿客 hongke.changhong.com 小程序端常见 /gw/applet/login 推断实现，
   同时保底兼容环境变量 chmlck 直填抓包 token）

功能：
  1. 本地 code 服务获取微信 code
  2. /gw/applet/login 使用 code 换 token（推断接口，可回落手动 token）
  3. 每日签到（/gw/applet/aggr/signin?aggrId=661）
  4. Token 缓存与自动刷新
  5. PushPlus / 企业微信推送
  6. 品赞代理，业务请求优先代理，失败直连兜底

环境变量：
  chmlck            抓包 token（多个 & / 换行分隔），留空走 code 自动登录
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（签到日历只读验证用签到接口消息判断），失效自动 code 刷新"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            resp = api_post(server, f"{BASE_URL}/gw/applet/aggr/signin/calendar?aggrId=661", cache_token, proxies, {})
            if resp.get("status_code") == 200:
                print("✅ [缓存] token 有效")
                return cache_token, None
            # 接口不存在（404 等）时退化为接受缓存 token
            if resp.get("status_code") in (404, 405) and not resp.get("message", "").startswith("JSON解析失败"):
                print("⚠️ [缓存] 验证接口不可用，直接使用缓存 token")
                return cache_token, None
        except Exception as exc:
...
def is_success(resp: Dict[str, Any]) -> bool:
    """源脚本判定：status_code == 200 或 message 包含「成功」"""
...
def do_sign(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    ...
    if is_success(resp):
        msg = resp.get("message") or "签到成功"
        print(f"✅ [签到] {msg}")
        return str(msg)

    msg = resp.get("message") or json_preview(resp, 300)
    print(f"❌ [签到] 签到失败：{msg}")
    return f"签到失败：{msg}"


def load_manual_tokens() -> List[str]:
    """解析环境变量 chmlck：抓包 token（多个 & / 换行分隔），留空则走 code 登录"""
...
def main() -> None:
    ...
    send_pushplus("❄️ 长虹美菱任务完成", build_notify(results))
