...
功能：
  ...
  2. 登录接口使用 code 换 authorization token（推断接口）
  3. 每日签到任务（/award/114）
  4. 查询会员积分（/member usableScore）
  5. PushPlus 推送
...
⚠️ 登录接口为推断，未经真机验证，失败请抓包核对
   （源脚本为抓包 authorization Bearer token 型，仅含 /award/114 与 /member 两个接口）
"""
...
BASE_URL = "https://vip.heytea.com/api/service-member/vip/task"
LOGIN_URL = f"{BASE_URL}/login"  # ⚠️ 推断接口
SIGN_URL = f"{BASE_URL}/award/114"
MEMBER_URL = f"{BASE_URL}/member"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "xichacookie.json")
...
def common_headers(token: str | None = None) -> Dict[str, str]:
    headers = {
        ...
        "Referer": f"https://servicewechat.com/{APPID}/1012/page-frame.html",
    }
    if token:
...
def extract_token(data: Any) -> str | None:
    ...
    if isinstance(inner, dict):
        ...
        user = inner.get("user") or inner.get("userMain")
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    ...
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            member_resp = api_get(server, MEMBER_URL, cache_token, proxies)
            if isinstance(member_resp.get("data"), dict) and member_resp.get("data"):
                print("✅ [缓存] token 有效")
...
def daily_sign(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """每日签到任务：POST /award/114（源脚本直接打印签到响应）"""
    resp = api_post(server, SIGN_URL, token, proxies, {})
    if resp.get("message") or resp.get("msg"):
        message = resp.get("message") or resp.get("msg")
    elif resp.get("data"):
        message = json_preview(resp.get("data"), 200)
    else:
        message = json_preview(resp, 200)
    print(f"📝 [签到] 任务响应: {message}")
    return str(message)


def query_points(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """查询会员积分：GET /member data.usableScore"""
    resp = api_get(server, MEMBER_URL, token, proxies)
    usable_score = safe_data(resp).get("usableScore")
    if usable_score is None:
        print(f"⚠️ [积分] 获取积分失败: {json_preview(resp, 300)}")
        return "-"

    print(f"⭐ [积分] 拥有积分: {usable_score}")
    return str(usable_score)
...
def main() -> None:
    ...
    send_pushplus("🧋 喜茶任务完成", build_notify(results))
