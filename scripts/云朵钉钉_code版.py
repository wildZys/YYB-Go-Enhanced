...
"""
云朵叮叮（云朵钉钉）有赞商城小程序动态 code 版

功能：
  1. 本地 code 服务获取微信 code
  2. /wscshop/weapp/authorize.json 使用 code 换 token（grantType=yz_union）
  3. 查询小云朵积分（签到前后各一次）
  4. 获取签到活动并每日签到（checkinV2）
  5. PushPlus 推送
...
KDT_ID = "152281203"
USER_VERSION = "2.213.1"
CLIENT_ID = "4d65249d377b2c3ed8"
CLIENT_SECRET = "1cdc05151d64f3a4a6ebd0e9de64422a"
GRANT_TYPE = "yz_union"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ydddcookie.json")
...
def build_api_url(api_path: str, access_token: str, query: Dict[str, Any] | None = None) -> str:
    """照源脚本 buildUrl：store_id/app_id/kdt_id/access_token 固定参数 + 额外 query"""
    params: Dict[str, Any] = {
        "store_id": "",
        "app_id": APPID,
        "kdt_id": KDT_ID,
    ...
    query_string = "&".join(f"{key}={quote(str(value), safe='')}" for key, value in params.items())
    return f"{API_BASE}/{api_path.lstrip('/')}?{query_string}"
...
def extract_session_id(data: Any) -> str:
    ...
    candidates = [data.get("sessionId"), data.get("session_id")]

    inner = data.get("data")
    if isinstance(inner, dict):
        candidates.extend([inner.get("sessionId"), inner.get("session_id")])
    ...
    try:
        ...
        if token:
            ...
            if isinstance(data, dict):
                ...
                nick_name = str(inner.get("nick_name") or inner.get("nickName") or "")
                mobile = inner.get("mobile") or ""
            print(f"👤 [登录] {nick_name} {mask_phone(mobile)}".rstrip())
            return token, data
...
def api_get(server: str, url: str, token: str, proxies: Dict[str, str] | None) -> Dict[str, Any]:
    response = request_with_proxy(
        "GET",
        url,
        headers=common_headers(token, get_cached_session_id(server)),
        proxies=proxies,
        server=server,
    )
    try:
...
def save_session_id(server: str, session_id: str) -> None:
    if not session_id:
        return
    cache = load_token_cache()
    data = cache.get(server)
    if isinstance(data, dict):
        ...
        cache[server] = {"sessionId": session_id}
    save_token_cache(cache)


def remove_cached_token(server: str) -> None:
    """照源脚本 removeCachedToken：鉴权类签到失败时清缓存，下次运行重新 code 登录"""
    cache = load_token_cache()
    if server in cache:
        del cache[server]
        save_token_cache(cache)
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（积分信息接口验证），失效自动 code 刷新"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            points_resp = api_get(server, build_api_url("wscump/integral/user_points.json", cache_token), cache_token, proxies)
    ...
    save_session_id(server, extract_session_id(raw_login))
    return token, raw_login


def get_points(server: str, token: str, proxies: Dict[str, str] | None, label: str = "积分") -> Any:
    """照源脚本 getPoints：查询小云朵积分"""
    resp = api_get(server, build_api_url("wscump/integral/user_points.json", token), token, proxies)
    if resp.get("code") != 0:
        print(f"⚠️ [{label}] 查询失败: {resp.get('msg') or resp.get('message') or json_preview(resp, 300)}")
        return None
    data = safe_data(resp)
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # 获取签到活动信息
        page_resp = api_get(server, build_api_url("wscump/checkin/show_checkin_page_v2.json", token), token, proxies)
        checkin_id = ""
        ...
        if not checkin_id:
            ...
            sign_resp = api_get(
                server,
                build_api_url("wscump/checkin/checkinV2.json", token, {"checkinId": checkin_id}),
                token,
                proxies,
            )
            if sign_resp.get("code") == 0:
                ...
                for item in sign_data.get("list") or []:
                    infos = item.get("infos") if isinstance(item, dict) else {}
                    if not isinstance(infos, dict):
                        continue
                    award = str(infos.get("title") or infos.get("desc") or "")
                    if award:
                        awards.append(award)
                ...
                message = str(sign_resp.get("msg") or sign_resp.get("message") or json_preview(sign_resp, 300))
                if re.search(r"已签到|已经签到|重复|今日.*签|参与次数", message):
                    ...
                    print(f"❌ [签到] {result['signMsg']}")
                    if (
                        sign_resp.get("code") in (-1, 40010, 40009)
                        or re.search(r"登录|token|access", message, re.IGNORECASE)
                    ):
                        remove_cached_token(server)
                        print("⚠️ [签到] 鉴权异常，已清除缓存 token")

        # 签到后积分
        points_after = get_points(server, token, proxies, "签到后")
        result["pointsMsg"] = f"{points_after}小云朵" if points_after is not None else "查询失败"
...
def main() -> None:
    ...
    send_pushplus("☁️ 云朵叮叮（云朵钉钉）任务完成", build_notify(results))
