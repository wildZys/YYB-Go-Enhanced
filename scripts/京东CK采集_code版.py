...
功能：
  ...
  2. GET /mlogin/wxapp/login_lt 静默登录（code 换 pt_key/pt_pin Set-Cookie）
  3. 采集 JD_COOKIE 并缓存到本地 JSON（重复运行先验证缓存有效性）
  4. 京东用户信息接口验证缓存 cookie（只读）
  5. PushPlus 推送
...
PAGE_VERSION = "873"
LOGIN_URL = "https://wq.jd.com/mlogin/wxapp/login_lt"
JD_USER_INFO_URL = "https://me-api.jd.com/user_new/info/GetJDUserInfoUnion"  # 补充的只读缓存验证接口

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "jdcodecookie.json")

USER_AGENT = (
    "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) "
    "Mobile/15E148 MicroMessenger/8.0.49 NetType/WIFI Language/zh_CN miniProgram/" + APPID
)
...
def pick_cookie(set_cookie_text: str, name: str) -> str:
    match = re.search(r"(?:^|[;,\s])" + name + r"=([^;]+)", set_cookie_text or "")
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    ...
    try:
        ...
        if not isinstance(body, dict):
            body = {"raw": str(body)[:800]}

        # 从 Set-Cookie 提取 pt_key/pt_pin（优先用 requests 解析好的 cookie jar，再兜底原始头）
        set_cookie_raw = ""
        try:
            set_cookie_raw = ",\n".join(response.raw.headers.getlist("Set-Cookie"))
        except Exception:
            set_cookie_raw = response.headers.get("Set-Cookie", "")
        pt_key = response.cookies.get("pt_key") or pick_cookie(set_cookie_raw, "pt_key")
        pt_pin = response.cookies.get("pt_pin") or pick_cookie(set_cookie_raw, "pt_pin")
...
def verify_jd_cookie(jd_cookie: str, proxies: Dict[str, str] | None, server: str = "") -> bool:
    """京东用户信息只读接口验证 pt_key/pt_pin 是否仍有效"""
    try:
        resp = api_get(server, JD_USER_INFO_URL, jd_cookie, proxies,
                       headers={"User-Agent": USER_AGENT, "Cookie": jd_cookie, "Accept": "application/json"})
        if resp.get("ret") == "0" or resp.get("ret") == 0:
            return True
        inner = safe_data(resp)
        return bool(isinstance(inner, dict) and inner.get("userInfo"))
    except Exception as exc:
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 JD_COOKIE（用户信息接口验证），失效自动 code 刷新采集"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 JD_COOKIE")
        if verify_jd_cookie(cache_token, proxies, server):
            print("✅ [缓存] JD_COOKIE 有效")
            return cache_token, None
        print("⚠️ [缓存] JD_COOKIE 已失效，重新采集")
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    if token:
        ...
        result["cookieMsg"] = f"JD_COOKIE 有效: {mask(token)}"
        result["success"] = True
        return result

    try:
        if raw_login and isinstance(raw_login, dict) and raw_login.get("blocked"):
            result["error"] = str(raw_login.get("blocked"))
            return result

        result["error"] = f"采集失败: {json_preview(raw_login, 300)}"
        return result
...
def main() -> None:
    ...
    print()
    print("╔" + "═" * 50 + "╗")
    print("║ 🏁 京东Code采集任务执行完成              ║")
    print(f"║ ✅ 成功: {success_count:<39}║")
    print(f"║ ❌ 失败: {fail_count:<39}║")
    print(f"║ 🕒 结束时间: {now_text():<32}║")
    print("╚" + "═" * 50 + "╝")

    send_pushplus("📦 京东Code采集任务完成", build_notify(results))
