...
功能：
  1. 四端口本地服务获取微信 code
  2. /account-center/oauth/mini-app-login 使用 code 换 token
     —— 企迈 AES-GCM 加密登录（实证，同平台 qmai 脚本验证通过）；
     企迈全站接口（含签到/积分/用户信息等业务接口）统一走 AES-GCM 加密请求
  3. 获取用户信息（手机号）
  4. 每日签到（含 MD5 签名，签到活动 1103263769176125440）
  5. 查询当前积分 / 即将过期积分
  6. PushPlus 推送
  7. 品赞代理，业务请求优先代理，失败直连兜底

⚠️ 登录已改为企迈 AES-GCM 加密登录（实证，同平台 qmai/茉莉奶白 脚本验证）；
   业务端点路径沿用原值，未经真机验证，失败请抓包核对
...
CUSTOMER_CENTER_URL = f"{BASE_URL}/web/catering/crm/customer-center?appid={APPID}"
PERSONAL_INFO_URL = f"{BASE_URL}/web/catering/crm/personal-info"
SIGN_STATUS_URL = f"{BASE_URL}/web/cmk-center/sign/userSignStatistics"
SIGN_URL = f"{BASE_URL}/web/cmk-center/sign/takePartInSign"
POINTS_INFO_URL = f"{BASE_URL}/web/catering/crm/points-info"

# 签到活动ID
ACTIVITY_ID = "1103263769176125440"
# 签到门店/商家ID
STORE_ID = 215357
# 签名用固定 userId（照源脚本）
SIGN_USER_ID = 10086
...
COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mwwcookie.json")
...
def api_get(server: str, url: str, token: str, proxies: Dict[str, str] | None, payload: Dict[str, Any] | None = None) -> Dict[str, Any]:
...
def get_sign(timestamp: int, user_id: int) -> str:
    """签到签名：参数按 key 排序拼接后接 reversed(activityId)，MD5 大写"""
    params = {
    ...
    encoded = "&".join(f"{key}={params[key]}" for key in sorted_keys)
    encoded += "&key=" + ACTIVITY_ID[::-1]
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（客户中心接口验证），失效自动 code 刷新"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            center_resp = api_get(server, CUSTOMER_CENTER_URL, cache_token, proxies)
            if center_resp.get("status"):
                print("✅ [缓存] token 有效")
...
def get_user_info(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """获取用户信息，返回手机号"""
    data = api_get(server, PERSONAL_INFO_URL, token, proxies, {"appid": APPID})
    if data.get("status"):
        mobile = safe_data(data).get("mobilePhone", "")
        print(f"👤 [用户] {mobile} 登录成功~")
        return str(mobile)

    print(f"⚠️ [用户] 获取用户信息失败: {data.get('message') or json_preview(data, 300)}")
    return "-"


def get_sign_in_status(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """查询签到状态，未签到则自动签到"""
    data = api_post(server, SIGN_STATUS_URL, token, proxies, {
        "activityId": ACTIVITY_ID,
        "appid": APPID,
    })
    if not data.get("status"):
        msg = data.get("message") or json_preview(data, 300)
        print(f"⚠️ [签到] 查询签到状态失败: {msg}")
        return f"查询签到状态失败: {msg}"

    sign_data = safe_data(data)
    sign_days = sign_data.get("signDays", 0)
    sign_status = sign_data.get("signStatus")

    if sign_status == 1:
        print(f"✅ [签到] 今天已经签到过了")
        print(f"📅 [签到] 已连续签到{sign_days}天")
        return f"今天已签到 | 已连续签到{sign_days}天"

    wait_time = random.uniform(3.0, 5.0)
    print(f"⏳ [间隔] 签到前等待 {wait_time:.1f}s")
    sleep(wait_time)
    return sign(server, token, proxies)


def sign(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """签到（takePartInSign，带 MD5 签名）"""
    timestamp = int(time.time() * 1000)
    data = api_post(server, SIGN_URL, token, proxies, {
        "activityId": ACTIVITY_ID,
        "storeId": STORE_ID,
        "timestamp": timestamp,
        "signature": get_sign(timestamp, SIGN_USER_ID),
        "appid": APPID,
    })
    if data.get("status"):
        print("✅ [签到] 签到成功")
...
def points_info(server: str, token: str, proxies: Dict[str, str] | None) -> Tuple[str, str]:
    """积分查询，返回 (当前积分, 过期提示)"""
    data = api_post(server, POINTS_INFO_URL, token, proxies, {"appid": APPID})
    if not data.get("status"):
        print(f"⚠️ [积分] 获取积分失败: {data.get('message') or json_preview(data, 300)}")
        return "-", "-"

    points_data = safe_data(data)
    total_points = points_data.get("totalPoints", 0)
    soon_expired = points_data.get("soonExpiredPoints", 0)
    expired_time = points_data.get("expiredTime", "")

    print(f"💰 [积分] 当前积分：{total_points}")
    if to_float(soon_expired) > 0:
        expired_msg = f"{soon_expired}积分将在{expired_time}失效"
        print(f"⌛ [积分] {expired_msg}")
    else:
        expired_msg = "暂无过期积分"
        print(f"⌛ [积分] {expired_msg}")
    return str(total_points), expired_msg
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sleep(wait_time)
        result["mobile"] = get_user_info(server, token, proxies)

        wait_time = random.uniform(1.0, 1.5)
        print(f"⏳ [间隔] 等待 {wait_time:.1f}s")
        sleep(wait_time)
        result["signMsg"] = get_sign_in_status(server, token, proxies)

        wait_time = random.uniform(1.0, 1.5)
        print(f"⏳ [间隔] 等待 {wait_time:.1f}s")
        sleep(wait_time)
        result["points"], result["expiredMsg"] = points_info(server, token, proxies)
...
def main() -> None:
    ...
    send_pushplus("🥤 馬伍旺饮料厂任务完成", build_notify(results))
