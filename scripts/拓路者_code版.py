...
"""
拓路者小程序动态 code 版（有赞店铺）

功能：
  1. 本地 code 服务获取微信 code
  2. /wscshop/weapp/authorize.json 使用 code 换 accessToken（weapp_wsc 授权）
  3. 读取签到活动并每日签到（checkinV2）
  4. 查询当前积分
  5. PushPlus 推送
...
COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "yz19cookie.json")
...
def login_by_code(
    ...
    try:
        ...
        mobile = mask_phone(data.get("mobile") or "")
        if nickname or mobile:
            print(f"👤 [登录] {nickname} {mobile}".strip())

        token_info = {
            "accessToken": access_token,
            "sessionId": str(data.get("sessionId") or data.get("session_id") or ""),
            "kdtId": str(data.get("kdtId") or data.get("kdt_id") or KDT_ID),
            "cookie": str(data.get("cookie") or cookie or ""),
            "nickname": nickname,
            "mobile": str(data.get("mobile") or ""),
        }
        return token_info, resp
    except Exception as exc:
        print(f"❌ [登录] 请求异常: {exc}")
        return None, None
...
def api_get(
    ...
    token_info = token_info or {}
    url = build_url(api_path, token_info, query)
    response = request_with_proxy(
        "GET",
        url,
...
def api_post(
    ...
    response = request_with_proxy(
        "POST",
        url,
        headers=common_headers(token_info.get("sessionId", ""), token_info.get("cookie", ""), with_json=True),
        json=payload or {},
        proxies=proxies,
        server=server,
    )
...
def get_points(
    ...
    resp = api_get(server, POINTS_URL, token_info, proxies)
    if resp.get("code") != 0:
        raise RuntimeError(str(resp.get("msg") or resp.get("message") or json_preview(resp, 300)))
    data = safe_data(resp)
...
def get_checkin_info(
    ...
    resp = api_get(server, CHECKIN_PAGE_URL, token_info, proxies)
    if resp.get("code") != 0:
    ...
    print(f"📋 [签到] 签到活动: checkinId={checkin_id or '未获取'} isShow={data.get('isShow')}")
    return checkin_id
...
def do_checkin(
    ...
    resp = api_get(server, CHECKIN_URL, token_info, proxies, query={"checkinId": checkin_id})
    if resp.get("code") == 0:
        data = safe_data(resp)
        awards = ", ".join(
            award
    ...
    message = str(resp.get("msg") or resp.get("message") or json_preview(resp, 300))
    if re.search(r"已达最大参与次数|已签到|重复签到", message):
        print("✅ [签到] 今日已签到")
        return "今日已签到"
...
def login_with_cache(
    ...
    if cache_token:
        ...
        token_info = {
            ...
            "kdtId": str(cached.get("kdtId", "") or KDT_ID),
            "cookie": str(cached.get("cookie", "") or ""),
            "nickname": str(cached.get("nickname", "") or ""),
            "mobile": str(cached.get("mobile", "") or ""),
        }
        print("🔍 [缓存] 验证 token")
        try:
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        user = token_info.get("nickname") or ""
        mobile = token_info.get("mobile") or ""
        if raw_login and isinstance(raw_login, dict):
            data = safe_data(raw_login)
            user = str(data.get("nick_name") or data.get("nickName") or "") or user
            mobile = str(data.get("mobile") or "") or mobile
        result["user"] = f"{user} {mask_phone(mobile)}".strip() or "-"
...
def build_notify(results: List[Dict[str, Any]]) -> str:
    ...
    content = f"""⛰️ 拓路者小程序动态 code 任务结果
...
def main() -> None:
    ...
    send_pushplus("⛰️ 拓路者任务完成", build_notify(results))
