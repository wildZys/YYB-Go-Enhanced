...
功能：
  1. 四端口本地服务获取微信 code
  2. 青梅 mini-app-login 使用 code 换 qm-user-token
  3. 会员中心接口校验 token（crm/customer-center）
  4. 每日签到（cmk-center/sign/takePartInSign）
  5. 查询连续签到天数与下一阶段奖励
  6. 查询积分与即将过期积分
  7. PushPlus 推送
  8. 品赞代理，业务请求优先代理，失败直连兜底

⚠️ 登录接口为推断（复用同平台青梅 /web/account-center/oauth/mini-app-login 端点），
   未经真机验证，失败请抓包核对
...
SCENE = "1145"
PAGE_VERSION = "49"
ACTIVITY_ID = os.getenv("PAOTEA_ACTIVITY_ID", "983701274523176960")

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "paoteacookie.json")
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        print("🔐 [登录] 使用 code 换 token（青梅 mini-app-login，推断接口）")
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（会员中心接口验证），失效自动 code 刷新"""
...
def query_sign_days(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """查询连续签到天数与下一阶段奖励（照源脚本 userSignStatistics）"""
    try:
        resp = api_post(server, SIGN_STATS_URL, token, proxies, {
            "activityId": ACTIVITY_ID,
            "appid": APPID,
        })
        if not is_ok(resp):
            print(f"⚠️ [连续签到] 查询失败: {resp.get('message') or json_preview(resp, 200)}")
            return "-"

        data = safe_data(resp)
        sign_days = to_float(data.get("signDays"))
        print(f"📅 [连续签到] 已连续签到 {int(sign_days)} 天")

        reward_list = data.get("rewardList") if isinstance(data.get("rewardList"), list) else []
        for reward in reward_list:
            if not isinstance(reward, dict):
                continue
            if to_float(reward.get("signNum")) > sign_days:
                days_left = int(to_float(reward.get("signNum")) - sign_days)
                inner = reward.get("rewardList") if isinstance(reward.get("rewardList"), list) else []
                reward_name = "-"
                if inner and isinstance(inner[0], dict):
                    reward_name = str(inner[0].get("rewardName") or "-")
                print(f"🎁 [连续签到] 下一阶段奖励[{reward_name}]还需签到 {days_left} 天")
                return f"已连续签到 {int(sign_days)} 天，下一阶段奖励[{reward_name}]还需签到 {days_left} 天"

        print("🎉 [连续签到] 已达到所有签到奖励阶段")
        return f"已连续签到 {int(sign_days)} 天，已达到所有签到奖励阶段"
    except Exception as exc:
        print(f"⚠️ [连续签到] 查询异常: {exc}")
        return "-"


def query_points(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """查询积分与即将过期积分（照源脚本 points-info）"""
    try:
        resp = api_post(server, POINTS_INFO_URL, token, proxies, {"appid": APPID})
        if not is_ok(resp):
        ...
        total_text = str(total_points) if total_points is not None else "-"
        soon_expired = to_float(data.get("soonExpiredPoints"))
        expired_time = data.get("expiredTime") or ""
        print(f"💰 [积分] 当前积分: {total_text}")

        if soon_expired > 0:
            print(f"⏳ [积分] {int(soon_expired)} 积分将在 {expired_time} 失效")
            return f"{total_text}（{int(soon_expired)} 积分将在 {expired_time} 失效）"
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        # 查询新版签到状态，signStatus==1 表示今天已签（照源脚本 queryNewSign）
        stats = api_post(server, SIGN_STATS_URL, token, proxies, {
            "activityId": ACTIVITY_ID,
            "appid": APPID,
        })
        signed_today = False
        if is_ok(stats):
            signed_today = safe_data(stats).get("signStatus") == 1
            if signed_today:
                print("✅ [签到] 今天已经签到过了")
        else:
            print(f"⚠️ [签到] 查询签到状态失败: {stats.get('message') or json_preview(stats, 200)}")
        ...
        result["signDaysMsg"] = query_sign_days(server, token, proxies)
        result["pointsMsg"] = query_points(server, token, proxies)

        result["success"] = result["signMsg"] in ("签到成功", "今日已签到")
        return result
...
def main() -> None:
    ...
    send_pushplus("🍵 爷爷不泡茶任务完成", build_notify(results))
