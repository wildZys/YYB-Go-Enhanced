...
BASE_URL = "https://api.box.vipinfinity.com"
...
def common_headers(token: str | None = None) -> Dict[str, str]:
    headers = {
        ...
        "Client-Name": "default",
        "Referer": f"https://servicewechat.com/{APPID}/9/page-frame.html",
...
def get_user_info(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """获取用户信息，返回 姓名(手机号) 描述"""
    data = api_get(server, USER_INFO_URL, token, proxies)
    if data.get("code") != 0:
        msg = data.get("message") or json_preview(data, 300)
        print(f"⚠️ [用户] 获取用户信息失败: {msg}")
        return "-"

    user = safe_data(data).get("user") or {}
    name = user.get("name", "未知用户")
    phone = user.get("phone", "")
    user_msg = f"{name}({phone})" if phone else str(name)
    print(f"👤 [用户] {user_msg}")
    return user_msg


def sign(server: str, token: str, proxies: Dict[str, str] | None) -> Tuple[str, str]:
    """每日签到，返回 (签到结果, 连续天数描述)"""
    data = api_post(server, SIGN_URL, token, proxies, {})
    if data.get("code") != 0:
        msg = data.get("message") or "签到失败"
        print(f"⚠️ [签到] {msg}")
        return msg, "-"

    sign_data = safe_data(data)
    continuous_days = sign_data.get("continuous_days", 0)
    award_score = sign_data.get("award_score", 0)
    print(f"✅ [签到] 签到成功，积分+{award_score}")
    print(f"📅 [签到] 已连续签到{continuous_days}天")
    return f"签到成功，积分+{award_score}", f"已连续签到{continuous_days}天"


def points_info(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """查询当前积分（取最近一条资产记录的余额）"""
    data = api_get(server, SCORE_URL, token, proxies)
    if data.get("code") != 0:
        msg = data.get("message") or json_preview(data, 300)
        print(f"⚠️ [积分] 积分查询失败: {msg}")
        return "-"

    sign_data = safe_data(data)
    record_list = sign_data.get("list") or []
    if not record_list:
        print("⚠️ [积分] 暂无积分记录")
        return "-"

    score = record_list[0].get("after", 0)
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sleep(wait_time)
        sign_msg, continuous_msg = sign(server, token, proxies)
        result["signMsg"] = sign_msg
        result["continuousMsg"] = continuous_msg

        wait_time = random.uniform(1.0, 1.5)
        print(f"⏳ [间隔] 等待 {wait_time:.1f}s")
        sleep(wait_time)
        result["points"] = points_info(server, token, proxies)
...
def main() -> None:
    ...
    send_pushplus("🎨 开天工作室任务完成", build_notify(results))
