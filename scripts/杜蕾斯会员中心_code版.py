...
"""
杜蕾斯会员中心微信小程序动态 code 版（appid wxe11089c85860ec02，vip.ixiliu.cn 后端）

功能：
  1. 本地 code 服务获取微信 code
  2. /mp/user/login 使用 code 换 Access-Token（缓存复用，失效自动刷新）
     ⚠️ 登录接口为推断，未经真机验证，失败请抓包核对
     （源脚本为抓包 Access-Token 型，无登录调用；按小程序常见契约推断
       POST /mp/user/login {code, sid, platform}，响应含 token/Access-Token 字段）
  3. 查询积分（/mp/user/info，可用积分 points / 总积分 points_total）
  4. 每日签到得安心币（/mp/sign/applyV2）
  5. PushPlus 推送
...
API_HOST = "https://vip.ixiliu.cn"

LOGIN_URL = f"{API_HOST}/mp/user/login"
USER_INFO_URL = f"{API_HOST}/mp/user/info"
SIGN_URL = f"{API_HOST}/mp/sign/applyV2"

SID = "10006"
PLATFORM = "MP-WEIXIN"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dlscookie.json")
...
def common_headers(token: str | None = None) -> Dict[str, str]:
    headers = {
        ...
        "sid": SID,
        "Referer": f"https://servicewechat.com/{APPID}/25/page-frame.html",
    }
    if token:
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """POST /mp/user/login（推断接口），token 放 Access-Token 头（与源脚本一致）"""
    try:
        ...
        if data.get("status") not in (200, None) and not extract_token(data):
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        # 查询积分（GetUserInfo：可用积分 points，总积分 points_total）
        user_resp = api_get(server, USER_INFO_URL, token, proxies)
        if user_resp.get("status") == 200:
            user_info = safe_data(user_resp).get("userInfo") or {}
            points = user_info.get("points")
            points_total = user_info.get("points_total")
            result["pointsMsg"] = f"可用积分 {points}，总积分 {points_total}"
            print(f"💰 [积分] {result['pointsMsg']}")
        else:
            print(f"⚠️ [积分] 查询积分失败: {user_resp.get('message') or json_preview(user_resp, 200)}")

        wait_time = random.randint(1, 3)
        print(f"⏳ [签到] 签到前等待 {wait_time}s")
        sleep(wait_time)

        # 签到得安心币（Sign：GET /sign/applyV2）
        sign_resp = api_get(server, SIGN_URL, token, proxies)
        if sign_resp.get("status") == 200:
            result["signMsg"] = str(sign_resp.get("message") or "签到成功")
            print(f"✅ [签到] 签到结果: {result['signMsg']}")
        else:
...
def main() -> None:
    ...
    send_pushplus("🧴 杜蕾斯会员中心任务完成", build_notify(results))
