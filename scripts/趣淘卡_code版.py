...
功能：
  1. 四端口本地服务获取微信 code
  2. /api/login 使用 code 换 token（token 请求头）
  3. 每日签到
  4. 查询会员资料
  5. PushPlus 推送
...
BASE_URL = "https://taotaoka.languoyun.cn"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "qutaokacookie.json")
...
def api_post(server: str, url: str, form_body: Dict[str, Any] | None, token: str | None,
             proxies: Dict[str, str] | None) -> Dict[str, Any]:
    body = "&".join(f"{k}={quote(str('' if v is None else v), safe='')}" for k, v in (form_body or {}).items())
    response = request_with_proxy(
        "POST", url, headers=common_headers(token), data=body, proxies=proxies, server=server,
    )
    try:
        return response.json()
    except Exception:
        return {"status": -1, "msg": f"JSON解析失败: {response.text[:300]}"}
...
def is_auth_error(text: str) -> bool:
    return bool(__import__("re").search(r"登录|token|未授权|未登录|失效|过期|重新|401", str(text or ""), __import__("re").IGNORECASE))


class AccountSession:
    """单个 code 服务地址的会话: token（token 请求头）"""
    ...
    def login(self) -> None:
        ...
        res = api_post(self.server, f"{BASE_URL}/api/login",
                       {"code": code, "nickname": "", "headimg": "", "pid": ""}, None, None)
        if not is_ok(res):
            raise RuntimeError(f"登录失败: {msg_of(res)}")
        # data 本身就是 token 字符串（不是对象）
        self.token = str(res.get("data") or "")
        if not self.token:
    ...
    def query_user(self) -> str:
        res = api_post(self.server, f"{BASE_URL}/api/user", {}, self.token, None)
        if is_ok(res):
            data = safe_data(res)
            self.nickname = data.get("nickname") or self.nickname
            bits = [self.nickname] if self.nickname else []
            for key in ("integral", "points", "score", "sign_days"):
                if data.get(key) is not None:
                    bits.append(f"{key}={data[key]}")
            msg = " ".join(bits) or json_preview(data, 120)
            print(f"👤 [会员] {msg}")
            return msg
    ...
    def sign(self, retry: bool = True) -> str:
        res = api_post(self.server, f"{BASE_URL}/api/sign_add", {}, self.token, None)
        if is_ok(res):
            print("✅ [签到] 签到成功")
            return "签到成功"
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result["userMsg"] = session.query_user()
