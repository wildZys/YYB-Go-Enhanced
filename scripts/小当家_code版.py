...
功能：
  1. 四端口本地服务获取微信 code
  2. /app/login/wechatLogin 使用 code 换 token（微信静默登录）
  3. 每日签到（含已签到判定与未注册提示）
  4. 会话失效自动重新登录重试
  5. PushPlus 推送
...
USER_AGENT = (
    ...
    "Safari/537.36 MicroMessenger/8.0.48.2580(0x28003036) MiniProgramEnv/android"
)
...
def extract_token(data: Any) -> str | None:
    ...
    if isinstance(inner, dict):
        candidates.extend([inner.get("token"), inner.get("accessToken"), inner.get("access_token")])
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        print("🔐 [登录] 使用 code 换 token（/app/login/wechatLogin）")
...
def is_ok(resp: Dict[str, Any]) -> bool:
    """该后端的成功判定：code==0 / code==200 / success==true"""
    try:
        code = int(resp.get("code"))
    except (TypeError, ValueError):
        code = None
    return code in (0, 200) or resp.get("success") is True


def msg_of(resp: Dict[str, Any]) -> str:
    return str(resp.get("msg") or resp.get("message") or json_preview(resp, 200))
...
def is_not_registered(text: str) -> bool:
    """账号态：这个微信号还没在该平台注册/绑定 —— 不是脚本缺陷"""
    return bool(re.search(r"未注册|未绑定|请先注册|请先绑定|not regist", str(text or ""), re.I))


def is_auth_error(text: str) -> bool:
    return bool(re.search(r"登录|token|未授权|未登录|失效|过期|重新|401", str(text or ""), re.I))
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """优先使用缓存 token（签到接口验证，无独立用户信息接口），失效自动 code 刷新"""
    cache_token = get_cached_token(server)
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            sign_resp = api_post(server, SIGN_URL, cache_token, proxies, {})
            if is_ok(sign_resp) or is_already_done(msg_of(sign_resp)):
                print("✅ [缓存] token 有效")
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if is_ok(sign_resp):
            ...
            print(f"✅ [签到] {result['signMsg']}")
        elif is_not_registered(msg_of(sign_resp)):
            result["signMsg"] = f"{msg_of(sign_resp)}（该微信号还没在该平台注册会员，先在小程序里注册一次再跑）"
            print(f"⚠️ [签到] {result['signMsg']}")
        elif is_auth_error(msg_of(sign_resp)):
            print("🔁 [签到] 会话失效，重新登录后重试")
            code = get_code(server)
            if not code:
                result["error"] = "重新获取 code 失败"
                return result
            new_token, raw_login = login_by_code(server, code, proxies)
            if not new_token:
                result["error"] = f"重新登录失败: {json_preview(raw_login)}"
                return result
            token = new_token
            set_cached_token(server, token, datetime.fromtimestamp(time.time() + 24 * 3600).isoformat())
            sign_resp = api_post(server, SIGN_URL, token, proxies, {})
            if is_ok(sign_resp):
                result["signMsg"] = "签到成功"
                print(f"✅ [签到] {result['signMsg']}")
        ...
        result["success"] = is_ok(sign_resp) or is_already_done(msg_of(sign_resp)) or "已签到" in result["signMsg"]
        return result
...
def main() -> None:
    ...
    send_pushplus("🏠 小铛家任务完成", build_notify(results))
