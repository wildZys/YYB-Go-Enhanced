...
功能：
  1. 四端口本地服务获取微信 code
  2. /login 使用 code 换 token（⚠️ 登录接口为推断，未经真机验证，失败请抓包核对）
  3. 每日签到（signin/addSignIn）
  4. 查询用户信息与余额（user/index）
  5. Token 缓存与自动刷新
  6. PushPlus / 企业微信推送
...
BASE_URL = "https://wt.api.5tan.com/api"
...
def common_headers(token: str | None = None) -> Dict[str, str]:
    headers = {
        ...
        "xweb_xhr": "1",
        "Referer": f"https://servicewechat.com/{APPID}/218/page-frame.html",
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        sign_resp = api_post(server, SIGN_URL, token, proxies, {"platform": 1})
        if sign_resp.get("code") == 200:
            sign_data = safe_data(sign_resp)
            if sign_data.get("title") == "签到成功~":
                result["signMsg"] = "签到成功"
                print(f"✅ [签到] {result['signMsg']}")
            else:
                result["signMsg"] = f"签到失败，{sign_data.get('content') or json_preview(sign_data, 200)}"
                print(f"❌ [签到] {result['signMsg']}")
        else:
        ...
        print(f"⏳ [延迟] 查询用户信息前等待 {wait_time}s")
        sleep(wait_time)

        user_resp = api_get(server, USER_INFO_URL, token, proxies)
        if user_resp.get("code") == 200:
            user_data = safe_data(user_resp)
            nickname = user_data.get("nick_name") or "-"
            money = user_data.get("money", 0)
            result["nickname"] = str(nickname)
            result["balance"] = str(money)
            print(f"🙍 [用户] 用户【{nickname}】余额：{money}元")
        else:
            print(f"⚠️ [用户] 获取用户信息失败，{user_resp.get('msg') or json_preview(user_resp, 300)}")
...
def main() -> None:
    ...
    send_pushplus("♻️ 捂碳星球任务完成", build_notify(results))
