...
功能：
  ...
  2. 使用 code 换 token（/mobile/wxAppLogin）
  3. 查询用户信息
  4. 从个人中心模板获取签到活动 activityId
  5. 每日签到（得积分）
  6. 查询当前积分
  7. Token 本地缓存与自动刷新
  8. PushPlus 推送
...
⚠️ 登录接口为推断（同平台 mall-mobile-v6.vecrp.com 小程序通用 /mobile/wxAppLogin），
   未经真机验证，失败请抓包核对。
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    """⚠️ 推断接口：同平台 vecrp 小程序通用 /mobile/wxAppLogin"""
    try:
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    ...
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            user_resp = api_get(server, f"{USER_INFO_URL}?shopId={SHOP_ID}", cache_token, proxies)
            if is_ok(user_resp):
                print("✅ [缓存] token 有效")
...
def get_activity_id(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    """从个人中心模板组件里解析签到活动 activityId"""
    try:
        resp = api_get(server, f"{TEMPLATE_URL}?shopId={SHOP_ID}", token, proxies)
        if not is_ok(resp):
            print(f"⚠️ [活动] 获取模板失败: {err_msg(resp)}")
            return ""

        views = (resp.get("result") or {}).get("views") or []
        component = next((v for v in views if isinstance(v, dict) and v.get("componentId") == SIGN_COMPONENT_ID), None)
        if not component:
            print(f"⚠️ [活动] 未找到 componentId 为 {SIGN_COMPONENT_ID} 的组件")
            return ""
        ...
        for img in imgs:
            ...
            if link.get("path") == SIGN_PAGE_PATH:
                ...
                if activity_id:
                    print(f"✅ [活动] activityId 已获取: {activity_id}")
                    return activity_id

        print("⚠️ [活动] 未找到签到活动 activityId")
        return ""
    except Exception as exc:
        print(f"⚠️ [活动] 获取活动ID异常: {exc}")
        return ""
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        # 用户信息
        user_resp = api_get(server, f"{USER_INFO_URL}?shopId={SHOP_ID}", token, proxies)
        if is_ok(user_resp):
            customer = (user_resp.get("result") or {}).get("customer") or {}
            name = customer.get("customerName") or "未知用户"
            mobile = str(customer.get("mobile") or "")
            hidden_mobile = f"{mobile[:3]}***{mobile[-3:]}" if len(mobile) >= 6 else mobile
            result["userMsg"] = f"{name}({hidden_mobile})"
        ...
        # 获取活动ID
        activity_id = get_activity_id(server, token, proxies)

        sleep(random.randint(1, 3))

        # 签到
        if not activity_id:
            result["signMsg"] = "未获取到 activityId，跳过签到"
            ...
            if is_ok(sign_resp):
                integral = ((sign_resp.get("result") or {}).get("integral")) or 0
                result["signMsg"] = f"签到成功，积分+{integral}"
        ...
        # 积分查询
        point_resp = api_get(server, f"{POINT_URL}?shopId={SHOP_ID}", token, proxies)
        if is_ok(point_resp):
            rows = point_resp.get("result") or []
            score = rows[0].get("score", "未知") if isinstance(rows, list) and rows else "未知"
            result["pointMsg"] = f"当前积分: {score}"
...
def main() -> None:
    ...
    send_pushplus("👟 特步会员中心任务完成", build_notify(results))
