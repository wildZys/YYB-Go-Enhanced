...
"""
贝因美小程序动态 code 版（有赞平台）

⚠️ 登录接口为推断，未经真机验证，失败请抓包核对（采用有赞 UIC 通用登录 auth.json）

功能：
  1. 本地 code 服务获取微信 code
  2. uic.youzan.com/passport/general/auth.json 使用 code 换 token（有赞 UIC 通用登录）
  3. 会话校验（wscaccount/api/authorize/data.json）
  4. 每日签到（wscump/checkin/checkinV2，checkinId=2845584）
  5. Token 缓存与自动刷新
  6. PushPlus / 企业微信推送
...
KDT_ID = "43704621"
CHECKIN_ID = "2845584"
ACCESS_TOKEN_FIXED = "b60985830b869f705a8bf0a784a07d"
USER_VERSION = "2.175.7.201"

COOKIE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "bymcookie.json")
        ...
        headers["extra-data"] = json.dumps(
            {
                ...
                "client": "weapp",
                "bizEnv": "",
                "uuid": "kcLq9rh5f7rAd1o1721544429919",
...
def build_h5_url(api_path: str, query: Dict[str, Any] | None = None) -> str:
    params: Dict[str, Any] = {
        "app_id": APPID,
        "kdt_id": KDT_ID,
        "access_token": ACCESS_TOKEN_FIXED,
        "appId": APPID,
        "kdtId": KDT_ID,
    }
    if query:
...
def extract_token(data: Any) -> str | None:
    """从有赞 UIC 登录响应中提取 accessToken/sessionId（业务实际使用 sid）"""
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        ...
        response = request_with_proxy(
            "POST",
            f"{UIC_LOGIN_URL}?kdt_id={quote(KDT_ID)}&app_id={quote(APPID)}",
...
def login_with_cache(server: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    ...
    if cache_token:
        print("🔍 [缓存] 验证 token")
        try:
            session_resp = api_get(server, build_h5_url(SESSION_PATH), cache_token, proxies)
            if session_resp.get("code") == 0:
                user_info = safe_data(session_resp).get("userInfo")
                if isinstance(user_info, dict) and user_info.get("hasLogin"):
                    print("✅ [缓存] token 有效")
...
def get_user_info(server: str, token: str, proxies: Dict[str, str] | None) -> Tuple[bool, str]:
    """获取用户信息，返回 (是否有效, 账号描述)"""
    try:
        resp = api_get(server, build_h5_url(SESSION_PATH), token, proxies)
        if resp.get("code") != 0:
            print(f"⚠️ [用户] 会话校验失败: {json_preview(resp, 300)}")
            return False, ""
        user_info = safe_data(resp).get("userInfo")
        if not isinstance(user_info, dict) or not user_info.get("hasLogin"):
            print("⚠️ [用户] sid 失效")
            return False, ""

        mobile = str(user_info.get("mobile") or "")
        nick_name = str(user_info.get("nickname") or "用户")
        hidden_mobile = f"{mobile[:3]}***{mobile[-3:]}" if len(mobile) >= 6 else mobile
        print(f"✅ [用户] {nick_name}({hidden_mobile})")
        return True, f"{nick_name}({hidden_mobile})"
    except Exception as exc:
        print(f"⚠️ [用户] 获取用户信息异常: {exc}")
        return False, ""
...
def do_sign(server: str, token: str, proxies: Dict[str, str] | None) -> str:
    resp = api_get(
        server,
        build_h5_url(CHECKIN_PATH, {"checkinId": CHECKIN_ID}),
        token,
        proxies,
    )
    if resp.get("code") == 0:
        gain = ""
        data = safe_data(resp)
        checkin_list = data.get("list")
        if isinstance(checkin_list, list) and checkin_list and isinstance(checkin_list[0], dict):
            infos = checkin_list[0].get("infos")
            if isinstance(infos, dict):
                gain = str(infos.get("title") or "")
        print(f"✅ [签到] 签到成功，+{gain}")
        return f"签到成功，+{gain}"

    msg = str(resp.get("msg") or resp.get("message") or json_preview(resp, 300))
    print(f"⚠️ [签到] {msg}")
    return f"今日已签到或不满足条件（{msg}）"
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ok, user_desc = get_user_info(server, token, proxies)
        if not ok:
            result["error"] = "sid 失效"
            return result

        result["userMsg"] = user_desc

        wait_time = random.uniform(2, 2.5)
...
def main() -> None:
    ...
    send_pushplus("🍼 贝因美小程序任务完成", build_notify(results))
