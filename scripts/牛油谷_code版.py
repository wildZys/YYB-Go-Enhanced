...
功能：
  1. 四端口本地服务获取微信 code
  2. /api/user/wxlogin 使用 code 换 token
  3. 每日签到（code==-2 视为已签到成功语义）
  4. 查询会员资料/积分
  5. PushPlus 推送
...
BASE_URL = "https://app.niuyougu.com.cn"
LOGIN_URL = f"{BASE_URL}/api/user/wxlogin"
...
def form_encode(payload: Dict[str, Any]) -> str:
    return "&".join(f"{k}={quote(str('' if v is None else v), safe='')}" for k, v in payload.items())
...
def api_post(server: str, url: str, form_body: Dict[str, Any], proxies: Dict[str, str] | None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"code": -999, "msg": f"JSON解析失败: {response.text[:300]}"}
...
def is_auth_error(text: str) -> bool:
    return bool(__import__("re").search(r"登录|token|未授权|失效|过期|重新", str(text or ""), __import__("re").IGNORECASE))


class AccountSession:
    """单个 code 服务地址的会话: token + nickname（token 在 body 里，不在请求头）"""
    ...
    def login(self) -> None:
        ...
        res = api_post(self.server, LOGIN_URL, {"token": "", "code": code, "channelId": 0}, None)
        if not is_ok(res):
        ...
        if not self.token:
            raise RuntimeError(f"登录未返回 token: {json_preview(res, 200)}")

        cache = load_token_cache()
        cache[self.server] = {"token": self.token, "nickname": self.nickname, "updateTime": datetime.now().isoformat()}
        save_token_cache(cache)
        print(f"✅ [登录] 登录成功{': ' + self.nickname if self.nickname else ''}")
    ...
    def ensure_login(self) -> None:
        ...
        if cached.get("token"):
            ...
            ok, _ = self.query_user(need_log=False)
            if ok:
                print("✅ [缓存] 使用缓存token")
                return
    ...
    def query_user(self, need_log: bool = True) -> Tuple[bool, str]:
        res = api_post(self.server, USER_URL, {"token": self.token}, None)
        if not is_ok(res):
            if need_log:
                print(f"⚠️ [会员] 读取资料失败: {msg_of(res)}")
            return False, "-"
        data = safe_data(res)
        ...
        for key in ("integral", "points", "score", "balance", "coin"):
            if data.get(key) is not None:
                bits.append(f"{key}={data[key]}")
        if need_log:
            print(f"👤 [会员] {' '.join(bits) or json_preview(data, 120)}")
        return True, " ".join(bits) or "-"

    def sign(self, retry: bool = True) -> str:
        res = api_post(self.server, SIGN_URL, {"token": self.token, "eid": 1, "_t": int(time.time() * 1000)}, None)
        if is_ok(res):
            print("✅ [签到] 签到成功")
            return "签到成功"
        if res.get("code") == CODE_ALREADY_SIGNED or is_already_done(msg_of(res)):
            print(f"✅ [签到] 今日已签到（{msg_of(res)}）")
            return f"今日已签到（{msg_of(res)}）"
        if retry and is_auth_error(msg_of(res)):
...
def run_account(index: int, total: int, server: str) -> Dict[str, Any]:
    ...
    try:
        ...
        ok, user_msg = session.query_user()
        result["userMsg"] = user_msg

        result["signMsg"] = session.sign()
